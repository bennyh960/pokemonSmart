...
function drawSpray(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, bodyColor: string): void {
  const s = size / 16;
  // Nozzle
...
function drawPokeball(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, topColor: string): void {
  const cx = x + size / 2;
  const cy = y + size / 2;
  const r = size * 0.42;
...
function drawArrowUp(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string): void {
  const s = size / 16;
  // Background square
...
/** Draw a candy shape. */
function drawCandy(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string): void {
  const s = size / 16;
  // Wrapper ends
...
function drawVial(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string): void {
  const s = size / 16;
  // Cork
...
/** Draw a folded document/scroll (used for key documents). */
function drawDocument(
  ...
  ctx.fillRect(x + 2 * s, y + 1 * s, 10 * s, 13 * s);
  // Folded top-right corner
  ctx.fillStyle = accentColor;
  ctx.beginPath();
  ctx.moveTo(x + 9 * s, y + 1 * s);
  ctx.lineTo(x + 12 * s, y + 1 * s);
  ctx.lineTo(x + 12 * s, y + 4 * s);
  ctx.closePath();
  ctx.fill();
  // Fold crease (white triangle)
  ...
  ctx.fillRect(x + 4 * s, y + 7 * s, 6 * s, s);
  ctx.fillRect(x + 4 * s, y + 9 * s, 4 * s, s);
  // Border
  ctx.strokeStyle = accentColor;
  ctx.lineWidth = s;
  ctx.strokeRect(x + 2 * s, y + 1 * s, 10 * s, 13 * s);
  ctx.lineWidth = 1;
}

/** Draw a hexagonal core chip (used for NULL-X core fragments). */
function drawCore(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string): void {
  const cx = x + size / 2;
  const cy = y + size / 2;
  const s = size / 16;
  // Hexagon body
  ...
  ctx.fill();
  // Circuit lines (3 spokes from center)
  ...
  for (let i = 0; i < 3; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + size * 0.28 * Math.cos(angle), cy + size * 0.28 * Math.sin(angle));
    ctx.stroke();
  }
  ctx.lineWidth = 1;
}
...
const ITEM_ICONS: Record<string, (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => void> = {
  ...
  // Key story items
  'secret-doc-act1': (ctx, x, y, s) => drawDocument(ctx, x, y, s, '#f0e8c8', '#c0a840'),
  'core-x1': (ctx, x, y, s) => drawCore(ctx, x, y, s, '#e03030'),
  'core-x2': (ctx, x, y, s) => drawCore(ctx, x, y, s, '#e07030'),
  'core-x3': (ctx, x, y, s) => drawCore(ctx, x, y, s, '#e0c030'),
  'core-x4': (ctx, x, y, s) => drawCore(ctx, x, y, s, '#40b060'),
  'core-x5': (ctx, x, y, s) => drawCore(ctx, x, y, s, '#30c0a0'),
  'core-x6': (ctx, x, y, s) => drawCore(ctx, x, y, s, '#3070e0'),
  'core-x7': (ctx, x, y, s) => drawCore(ctx, x, y, s, '#8030e0'),
  'core-x8': (ctx, x, y, s) => drawCore(ctx, x, y, s, '#404060'),
};
...
export const ITEM_ICON_STYLE: Record<string, { bg: string; stroke: string }> = {
  ...
  // Key story items
  'secret-doc-act1': { bg: 'rgba(192,168,64,0.18)', stroke: '#c0a840' },
  'core-x1': { bg: 'rgba(224,48,48,0.18)', stroke: '#e03030' },
  'core-x2': { bg: 'rgba(224,112,48,0.18)', stroke: '#e07030' },
  'core-x3': { bg: 'rgba(224,192,48,0.18)', stroke: '#e0c030' },
  'core-x4': { bg: 'rgba(64,176,96,0.18)', stroke: '#40b060' },
  'core-x5': { bg: 'rgba(48,192,160,0.18)', stroke: '#30c0a0' },
  'core-x6': { bg: 'rgba(48,112,224,0.18)', stroke: '#3070e0' },
  'core-x7': { bg: 'rgba(128,48,224,0.18)', stroke: '#8030e0' },
  'core-x8': { bg: 'rgba(64,64,96,0.25)', stroke: '#606090' },
};
 ...
 */
export function drawItemIcon(ctx: CanvasRenderingContext2D, itemId: string, x: number, y: number, size = 16): void {
